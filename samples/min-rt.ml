...
let rec normalize_vector v inv =
  ...
  v.(2) <- v.(2) /. n
in
let rec sgn x = if 0.0 < x then 1.0 else -1.0 in
...
let rec read_environ _ =
  ...
  cos_v.(1) <- cos v2;
  sin_v.(1) <- sin v2;
(*  debug_read_environ_v1_v2 v1 v2;*)
...
let rec read_nth_object n =
  ...
  if texture <> -1 then (
    ...
    xyz.(1) <- read_float ();
    xyz.(2) <- read_float ();
    let m_invert = 0.0 > read_float () in
    ...
    if isrot_p <> 0 then (
      ...
      cs_temp.(0) <- cs_temp.(12) *. cs_temp.(14);
      cs_temp.(1) <-
...
let rec read_object n =
  if n < 61 then if read_nth_object n then read_object (n + 1) else () else ()
in
let rec read_all_object _ = read_object 0 in
  ...
  if answera then 1
    ...
    let answerb =
      if 0.0 = l.(1) then false
        ...
        let d =
          if xor (o_isinvert m) (0.0 > l.(1)) then o_param_b m
          else -.o_param_b m
        in
    ...
    if answerb then 2
      ...
      let answerc =
        if 0.0 = l.(2) then false
          ...
          let d =
            if xor (o_isinvert m) (0.0 > l.(2)) then o_param_c m
            else -.o_param_c m
          in
...
let rec solver_second m l =
  ...
  let aa = if o_isrot m <> 0 then aa0 +. in_prod_co_objrot m l else aa0 in
  if 0.0 = aa then 0
  else
    let bb0 = 2.0 *. solver2nd_mul_b m l in
    let bb = if o_isrot m <> 0 then bb0 +. solver2nd_rot_b m l else bb0 in
    let cc0 = in_prod_sqr_obj m solver_w_vec in
    let cc1 =
      if o_isrot m <> 0 then cc0 +. in_prod_co_objrot m solver_w_vec else cc0
    in
...
let rec is_second_outside m =
  ...
  let w3 =
    if o_isrot m <> 0 then w2 +. in_prod_co_objrot m isoutside_q else w2
  in
  let s = 0.0 > w3 in
  not (xor (o_isinvert m) s)
in
  ...
  if range_primitive = -1 then ()
    ...
    (if range_primitive = 99 then solve_one_or_network 1 head
       ...
       if t <> 0 then
         ...
         if tp < tmin.(0) then solve_one_or_network 1 head else ()
       else ());
...
(*  debug_tracer viewpoint vscan tmin.(0);*)
  let t = tmin.(0) in
  if -0.1 < t then if t < 100000000.0 then true else false else false
in
...
let rec get_nvector_second_rot m p =
  ...
  nvector.(0) <-
    ...
    +. fhalf ((nvector_w.(1) *. o_param_r3 m) +. (nvector_w.(2) *. o_param_r2 m));
  nvector.(1) <-
    (nvector_w.(1) *. o_param_b m)
    +. fhalf ((nvector_w.(0) *. o_param_r3 m) +. (nvector_w.(2) *. o_param_r1 m));
  nvector.(2) <-
    (nvector_w.(2) *. o_param_c m)
    +. fhalf ((nvector_w.(0) *. o_param_r2 m) +. (nvector_w.(1) *. o_param_r1 m));
  normalize_vector nvector (o_isinvert m)
in
let rec get_nvector m p =
  ...
  else if o_isrot m <> 0 then get_nvector_second_rot m p
...
let rec utexture m p =
  ...
  if m_tex = 1 then
    ...
    let w7 =
      if abs_float w1 < 1.0e-4 then 15.0
      else
    ...
    let w8 =
      if abs_float w7 < 1.0e-4 then 15.0
      else
        let w6 = abs_float (w2 /. w4) in
        atan w6 *. (30.0 /. 3.1415927)
    in
...
(*  debug_raytracing nref energy crashed_p;*)
  ...
  else (()(*; debug_raytracing2 nref energy crashed_p*))
in
let rec write_rgb _ =
