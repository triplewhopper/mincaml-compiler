...
data Reg: IrTy -> Type where
    MkReg: (idx: Nat) -> (t: IrTy) -> Reg t

(.ty): Reg t -> IrTy
(.ty) (MkReg _ t) = t

Show (Reg t) where
    show (MkReg idx _) = "%\{show idx}"
...
mutual
    ...
    data GEP: IrTy -> SnocList (Value I32) -> IrTy -> Type where
        ...
        GEPLin: GEP t [< ConstV (ConstInt 0)] (Ptr t) 
    ...
    data Ir: {-(Maybe IrTy) -> -} Kind -> Type where
        ...
        GetElementPtr:  (t: IrTy) -> {r: IrTy} -> (dst: Reg (Ptr r)) ->  (src: Reg (Ptr t)) -> GEP t indices r -> Ir {- (Just (Ptr r)) -} Middle
...
Show (Ir k) where 
    show (Alloca t dst align) = "\{show dst} = alloca \{show t}, align \{show align} ; yields \{show (Ptr t)}"
    show (GetElementPtr t {r} dst src gep) = "\{show dst} = getelementptr \{show t}, \{show src.ty} \{show src}, \{show gep} ; yields \{show (Ptr r)}"
    show (Load t dst srcptr align) = "\{show dst} = load \{show t}, \{show srcptr.ty} \{show srcptr}, align \{show align}"
...
Show BasicBlock where 
    show (MkBB (MkReg idx Label) nRegAcc Nothing middles terminator) = unlines [show idx ++ ":"] ++ unlines (toList $ indent 4 <$> show <$> middles) ++ unlines [indent 4 $ show terminator]
    show (MkBB (MkReg idx Label) nRegAcc (Just initial) middles terminator) = unlines [
...
data IrCommand: (ty: Type) -> S -> (ty -> S) -> Type where
    ...
    CAlloca: {nRegs: Nat} -> (t: IrTy) -> (align: Nat) -> 
    ...
    CGetElementPtr: {nRegs: Nat} -> (t: IrTy) -> {r: IrTy} -> (src: Reg (Ptr t)) -> (gep: GEP t indices r) ->
        IrCommand (Reg (Ptr r)) (MkS nBlocks nRegs blockIdx initial middles blks p) (\dst => MkS nBlocks (S nRegs) blockIdx initial (middles:<(GetElementPtr t dst src gep)) blks p)
    CLoad: {nRegs: Nat} -> (t: IrTy) -> (srcptr: Reg (Ptr t)) -> (align: Nat) -> 
    ...
    CCall: {nRegs: Nat} -> (ret: IrTy) -> (f: IrFun (MkFunTy {n} argty (Just ret))) -> (args: Args argty) ->
    ...
    CAdd: {nRegs: Nat} -> (t: IrTy) -> (v1: Value t) -> (v2: Value t) -> 
...
deref: (t: IrTy) -> (idxes: List (Value I32)) -> Maybe IrTy
deref t [] = Just t
deref I1 _ = Nothing
deref I8 _ = Nothing
deref I32 _ = Nothing
...
gep: (t: IrTy) -> (idx: Value I32) -> (others: List (Value I32)) -> Type
gep t i others = case deref t others of Just r => GEP t (cast (i :: others)) (Ptr r); Nothing => Void


getelementptr: {nRegs: Nat} -> (t: IrTy) -> (src: Reg (Ptr t)) -> (indices: SnocList (Value I32)) -> {g: GEP t indices r} ->
IrCommand (Reg (Ptr r)) (MkS nBlocks nRegs blockIdx initial middles blks p) (\dst => MkS nBlocks (S nRegs) blockIdx initial (middles:<(GetElementPtr t dst src g)) blks p)
getelementptr t src [<] = CGetElementPtr t src GEPLin
...
test = do
    ...
    arr3 <- getelementptr (Array 200 I32) arr [< ConstV (ConstInt 3)]
    CStore I32 (ConstV (ConstInt 1)) r2 4
    CStore I32 (ConstV (ConstInt 2)) r3 4
    CBrLabel l
    CBlockEnd
...
-- runCmd (CPure x) = x
runCmd (CPhi {nRegs} t _) = MkReg nRegs t
runCmd (CAlloca {nRegs} t _) = MkReg nRegs (Ptr t)
runCmd (CGetElementPtr {nRegs} {r} t src gep) = MkReg nRegs (Ptr r)
runCmd (CLoad {nRegs} t _ _ ) = MkReg nRegs t
runCmd (CStore {nRegs} _ _ _ _) = ()
runCmd (CBrCond _ _ _) = ()
runCmd (CBrLabel _) = ()
runCmd (CRet _ _) = ()
runCmd CRetVoid = ()
runCmd (CCall {nRegs} ret _ _) = MkReg nRegs ret
runCmd (CCallVoid _ _) = ()
runCmd (CAdd {nRegs} t _ _) = MkReg nRegs t
...
fromTy (TyArray t) = Just (Ptr (fromMaybe I8 (fromTy t)))
...
-- gen: FunDef a arity' -> IrFun (MkFunTy args ret)
-- gen (MkFunDef (f, t) args formalFv body) = MkIrVoidFun f (snd args') ret (genBody body)
--     where
--         args': (p: Nat ** Vect p IrTy)
--         args' = mapMaybe id (fromTy <$> snd <$> args) 
--         ret: Maybe IrTy
--         genBody: Exp a -> (List1 BasicBlock, Ty)
--         gen (Let x _ U e2) =  
